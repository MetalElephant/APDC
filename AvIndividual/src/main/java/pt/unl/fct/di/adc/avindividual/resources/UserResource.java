...
public class UserResource {
	// TODO have a function create user rather than doing it manually everytime
	// (same for parcel)
	...
	public Response registerUser(RegisterData data) {
		...
		if (!data.validPasswordFormat())
			return Response.status(Status.BAD_REQUEST)
					.entity("Passwords should be at least 5 chars long, contain at least 1 "
							+ "letter, 1 upper case letter and 1 special character")
					.build();
		...
		Key redeemCodeKey = datastore.newKeyFactory().addAncestors(PathElement.of(USER, data.getCodeUser()))
				.setKind(CODE).newKey(data.code);
		Key generatedCodeKey = datastore.newKeyFactory().addAncestors(PathElement.of(USER, data.username)).setKind(CODE)
				.newKey(data.generateCode());
		...
		try {
			...
			// call some function to verify code and reward points, extra rewards for the
			// first 3 months
			// If indeed implements a points system update the other persons points as well

			// Create User and Code entity
			user = Entity.newBuilder(userKey).set(NAME, data.name).set(PASSWORD, DigestUtils.sha512Hex(data.password))
					.set(EMAIL, data.email).set(ROLE, data.role).set(MPHONE, data.mobilePhone)
					.set(HPHONE, data.homePhone).set(ADDRESS, data.address).set(NIF, data.nif)
					.set(VISIBILITY, data.visibility).set(PHOTO, uploadPhoto(data.username, data.photo))
					.set(POINTS, String.valueOf(points)).set(CTIME, Timestamp.now()).build();
			...
			Entity generatedCodeEntity = Entity.newBuilder(generatedCodeKey)
					.set(EXPTIME, Timestamp.of(expDate.getTime())).build();
			...
			if (user != null) {
				...
				if (hashedPwd.equals(DigestUtils.sha512Hex(data.password))) {
					...
					if (!canLogin(tokenEntity, tn)) {
						...
						tn.rollback();
						return Response.status(Status.CONFLICT).entity("User " + data.username + " already logged in.")
								.build();
					}

					AuthToken token = new AuthToken(data.username);

					tokenEntity = Entity.newBuilder(tokenKey).set(TOKENID, token.tokenID).set(TOKENUSER, token.username)
							.set(TOKENCREATION, token.validFrom).set(TOKENEXPIRATION, token.validTo).build();
		...
		try {
			...
			if (userToRemove == null) {
				...
				return Response.status(Status.NOT_FOUND)
						.entity("User to be removed " + data.usernameToRemove + " does not exist.").build();
			}
			...
			if (!canRemove(user, userToRemove)) {
				...
				return Response.status(Status.FORBIDDEN)
						.entity("User " + data.username + " unathourized to remove other User").build();
			}
	...
	public Response updateUser(UserUpdateData data) {
		...
		try {
			...
			if (userToUpdate == null) {
				...
				tn.rollback();
				return Response.status(Status.NOT_FOUND)
						.entity("User to be updated" + data.usernameToUpdate + " does not exist.").build();
			}
			...
			if (!canModify(data, user, userToUpdate))
				return Response.status(Status.FORBIDDEN).entity(
						"User " + data.username + " does not have authorization to change one or more attributes.")
						.build();

			userToUpdate = Entity.newBuilder(userUpdateKey).set(NAME, data.name)
					.set(PASSWORD, userToUpdate.getString(PASSWORD)).set(EMAIL, data.email).set(ROLE, data.role)
					.set(MPHONE, data.mobilePhone).set(HPHONE, data.homePhone).set(ADDRESS, data.address)
					.set(NIF, data.nif).set(VISIBILITY, data.visibility).set(POINTS, userToUpdate.getString(POINTS))
					.set(CTIME, userToUpdate.getTimestamp(CTIME)).build();
	...
	public Response updatePwd(PasswordUpdateData data) {
		...
		try {
			...
			if (user.getString(PASSWORD).equals(newPwd)) {
				...
				tn.rollback();
				return Response.status(Status.CONFLICT).entity("Old password can't be the same as new password.")
						.build();
			}
			...
			user = Entity.newBuilder(userKey).set(NAME, user.getString(NAME)).set(PASSWORD, newPwd)
					.set(EMAIL, user.getString(EMAIL)).set(ROLE, user.getString(ROLE))
					.set(MPHONE, user.getString(MPHONE)).set(HPHONE, user.getString(HPHONE))
					.set(ADDRESS, user.getString(ADDRESS)).set(NIF, user.getString(NIF))
					.set(VISIBILITY, user.getString(VISIBILITY)).set(CTIME, user.getTimestamp(CTIME)).build();
	...
	// TODO: we need a new data that has the user redeeming the reward, the owner
	// and the name
	@PUT
	@Path("/redeemReward")
	@Consumes(MediaType.APPLICATION_JSON)
	...
	private boolean canRemove(Entity user, Entity userToRemove) {
		/*
		 * String role1 = user.getString(ROLE); String role2 =
		 * userToRemove.getString(ROLE); String name1 = user.getString(USERNAME); String
		 * name2 = userToRemove.getString(USERNAME); if (role1.equals(USER) &&
		 * !name1.equals(name2)) { // can only remove themselves return false; } if
		 * (role1.equals(GBO) && !role2.equals(USER)) { // can only remove USER level
		 * return false; } if (role1.equals(GS) && role2.equals(SU)) { // can remove all
		 * but SU return false; }
		 */
		return true;
	}

	// TODO we won't need to check for null values if we get the old info so we will
	// only need canUpdateValues
	...
	private void removeUserCodes(String user, Transaction tn) {
		Query<Entity> codesQuery = Query.newEntityQueryBuilder().setKind(CODE)
				.setFilter(PropertyFilter.hasAncestor(datastore.newKeyFactory().setKind(USER).newKey(user))).build();
	...
	private List<String> buildQueryList(Query<Entity> query) {
		...
		users.forEachRemaining(userList -> {
			allUsers.add("Username: " + userList.getKey().getName() + " -|- Name: " + userList.getString(NAME)
					...
					+ " -|- Profile: " + userList.getString(VISIBILITY) + " -|- Password: "
					+ userList.getString(PASSWORD) + " -|- Address: " + userList.getString(ADDRESS) + " -|- Landphone: "
					+ userList.getString(HPHONE) + " -|- Mobile Phone: " + userList.getString(MPHONE) + " -|- NIF: "
					+ userList.getString(NIF) + " -|- Creation Time: " + userList.getTimestamp(CTIME).toString());
		});
