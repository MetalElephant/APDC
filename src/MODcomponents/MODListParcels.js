...
export default function ListParcels() {
    ...
    function parcelRemoval() {
        if (chosenParcel != null) {
            restCalls.deleteParcel(chosenParcel, owner).then(() => { restCalls.listParcels(); setIsParcelRemoved(true); setIsParcelNotRemoved(false); setDisplayMessage(true) }).catch(() => { setIsParcelRemoved(false); setIsParcelNotRemoved(true); setDisplayMessage(true) })
        } else {
