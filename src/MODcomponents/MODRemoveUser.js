...
export default function MODRemoveUser() {
    ...
    var userRemoving = JSON.parse(localStorage.getItem('token')).username
    ...
    function userToBeRemovedManager(e) {
        e.preventDefault();
        if (userToBeRemoved === userRemoving) {
            restCalls.deleteUser(userToBeRemoved).then(() => { history.push("/") })
